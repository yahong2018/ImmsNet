...
namespace Imms.Mes.Logic
{
    public class Picking
    {
        ...
        //生成领料计划
        //
        public MaterialPickingSchedule CreatePickingSchedule(ProductionOrder productionOrder, int qty = -1)
        {
            if (qty == -1)
            {
                qty = productionOrder.PlannedQty; //默认一次性全部领完
            }
            ...
            foreach (Bom bom in boms)
            {
                ...
                pickingBom.PlanQty = (bom.ComponentQty / productionOrder.PlannedQty) * qty;
            ...
            CommonDAO.UseDbContext((dbContext) =>
            {
                ...
                DataChangedNotifier.Notify(pickingSchedule, GlobalConstants.DML_OPERATION_INSERT);
            });
        ...
        //执行领料
        //
        public void PickMaterial(MaterialPickingOrder pickingOrder)
        {
            ...
            CommonDAO.UseDbContext((dbContext) =>
            {
                ...
                if (schedule.PickingBoms.Where(e => (e.PlanQty - e.PickedQty) > 0).Count() == 0) //物料已全部领完
                {
                ...
                DataChangedNotifier.Notify(schedule, GlobalConstants.DML_OPERATION_UPDATE);
            });
        }
    }
}
