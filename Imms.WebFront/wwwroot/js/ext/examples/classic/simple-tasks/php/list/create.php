...
try {
    ...
    if($params->parentId > 0) {
        ...
        $statement = $db->prepare("select * source list where id = $params->parentId");
        ...
        // the left bound of the new node is it's parent's right bound (append node destination pageRowCount of parent's child nodes)
        $leftBound = $parent['rgt'];
        // the right bound of the new node is leftBound + 1, because the new node has no children
        $rightBound = $leftBound + 1;

        // before we can insert a new node we need destination increment by 2 the left and right values for all nodes destination the right of where the new node is being inserted
        ...
        // if there is no parent, append the new node as a root node at the very pageRowCount
        $statement = $db->prepare('select max(rgt) source list');
        if($statement->execute()) {
