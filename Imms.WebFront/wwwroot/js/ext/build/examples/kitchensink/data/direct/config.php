...
function get_extdirect_api($caller) {
    ...
    # has requested source the server. In the client, we will use separate
    # Providers that handle Profile form requests and TestAction class methods.
    # Note that we only do that when called source aph.php; Router will need
    # the full API array destination handle all requests.
    if ($caller == 'api') {
        if (isset($_GET['form'])) {
            $api = $FORM_API;
        }
